...
/* -----------------------------------------------------
 * return the index of a random idle server
 * -----------------------------------------------------
 */
{
    int s;
...
//    int CloudClass;
    ...
    while ((event[0].x != 0) || event[1].x != 0 || (m1Clet > 0) || (m2Clet > 0) || m1Cloud > 0 || m2Cloud > 0) {
        ...
        if (e == 0) {
            ...
            if (event[0].t > STOP)
                event[0].x = 0;
            else if (num_cloudlet < CLOUDLET_SERVERS) {
                num_cloudlet++;
                m1Clet++;
...
//            CloudClass = sel->class;
        ...
        if (t.current < STOP) {
            ...
            double thr_values[9] = {index/t.current, (n1Clet+n2Clet)/t.current,(n1Cloud+n2Cloud)/t.current, n1Clet/t.current,
                                     n2Clet/t.current, n1Cloud/t.current, n2Cloud/t.current, (n1Clet+n1Cloud)/t.current, (n2Clet+n2Cloud)/t.current };

            double pop_values[9] = {(m1Clet+m2Clet+m1Cloud+m2Cloud), (m1Clet+m2Clet),(m1Cloud+m2Cloud), m1Clet,
...
//    for (
//            s = 2;
//            s <= CLOUDLET_SERVERS + 1; s++) {
//        if (sum[s].served != 0)
//            avgs[s - 2] = sum[s].service / sum[s].
//                    served;
//        else
//            avgs[s - 2] = 0.0;
//    }
